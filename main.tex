...
\mainmatter
	...
	\section{Methodology}	
		...
		\subsection{Inverse of the \glsentrylong {LD} matrix}
			...
			It is clearly show that the \gls{LD} matrix has a well-defined rank with a mean ``gap'' of 508,997,786,189.
			Therefore the choice of \gls{tSVD} for the regularization is appropriate.
			\begin{figure}
				\caption[Distribution of singular value of the LD matrix]{Distribution of singular value of the LD matrix, it can be observed that there is a sudden drop of the singular value, and the mean ``gap'' in the singular value were 508,997,786,189, suggesting that the LD matrix is well defined}
				\centering
				\includegraphics[width=0.5\textwidth]{pic/singular_value_distribution.png}
	...
	\section{Simulation}
		...
		\subsection{Quantitative Trait}
			...
			Therefore for \glspl{SNP} with a small effect, a larger effect size is given such that the effect size distribution in our simulation will follow that observed in the real life scenario\citep{Manolio2009}.
			...
			In order to determine a realistic and reasonable sample size for all simulation condition, we manually curated the sample size of all the studies presented on the \gls{GWAS} Catalog\cite{Welter2014} (version 2015-07-17).
			It was observed that the mean sample size of all published \gls{GWAS} is $\sim$ 7,200 samples. 
			Thus, we consider a simulation sample size of 7,200 should be comparable to general \gls{GWAS} studies, allow for a realistic simulation.
			Although a large sample size are generally required for GWAS, it might also worthwhile for one to test the performance when only small sample size is available.
			...
			To capture the full spectrum of trait complexity (e.g. Mendelian to Oligogenic to Complex Traits), we selected number of causal \glspl{SNP} to be $k\in(1,10,100)$ such that effect size$ \in(0,\frac{1}{k})$.To simplify the simulation procedure, all causal \glspl{SNP} were included within the data.
