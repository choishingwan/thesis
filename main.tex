...
\mainmatter
	...
	\section{Methodology}	
		...
		\subsection{Calculating the \glsentrylong{LD} matrix}
			...
			In reality, one can only obtain the \gls{LD} matrix based on a subset of the population (e.g. the 1000 genome project\cite{Project2012} or the HapMap project\cite{Altshuler2010}).
		...
		\subsection{Inverse of the \glsentrylong {LD} matrix}
			...
			Therefore, to obtain a meaningful solution, regularization techniques such as the Tikhonov Regularization (also known as Ridge Regression) and \gls{tSVD} has to be performed\cite{Neumaier1998}. 
			...
			This is because the \gls{SVD} routine has been implemented in the EIGEN C++ library \cite{eigenweb}, allowing us to implement the \gls{tSVD} method without much concern with regard to the detail of the algorithm. 
			...
			\begin{alignat}{2}
				...
				\label{eq:tsvd}				
			\end{alignat}
			where $\boldsymbol{\Sigma}_k$ equals to replacing the smallest $n-k$ singular value replaced by 0\cite{Hansen1987}. 
			...
			A problem with \gls{tSVD} however is that it only work when matrix $\boldsymbol{A}$ has a well determined numeric rank\cite{Hansen1987}.
			That is, \gls{tSVD} work best when there is a large gap between $\sigma_k$ and $\sigma_{k+1}$.
			
			\citeauthor{Hansen1987}(\citeyear{Hansen1987})\cite{Hansen1987} stated that for a matrix to have a well-determined numeric rank, the concept of rank usually make sense in the data. 
			
			In MATLAB, NumPy and GNU Octave, this threshold is taken to be $t=\epsilon\times\mathrm{max}(m,n)\times\mathrm{max}(\boldsymbol{\Sigma})$ where $\epsilon$ is the machine epsilon (the smallest number a machine can define as non-zero). 
			Using this tolerance threshold, we were able to solve the ill-posed \cref{eq:shrekEq}, therefore solving for $\boldsymbol{\hat{h^2}}$.
	...
	\section{Simulation}
		In order to assess the performance of the method under different scenarios, we performed a number of simulation to compare the performance of our algorithm with other existing softwares including \gls{gcta} and \gls{ldsc} 
