...
\setlength{\parskip}{0.75em}
	...
	\glsresetall
	% Long way for the study of Schizophrenia
	% identify heritability 
	% Genetic studyes
	% PGC
	
	% Need to identify the relative contribution to know what to do next
	
	% Understand the performance of SNP heritability estimation using summary statistics
	% SHREK is introduced
	% Better in extreme scenarios (robust)
	
	% Heritability estimation of Schizophrenia -> only ~20%
	% Some missing heriability
	% Gene sets associated with schizophrenia -> also perturbed in MIA 
	% Evidence converge, may act on same pathway (PSD+Calcium)
	% Continue research is needed
	
	% Treatment is important
	% Diet good 
	% Identify Sgk1 and PSD associated with diet
	% n-3 PUFA -> ??? -> Sgk1 -> Better phenotype outcome
	% Investigation is required
	
	% Finally, contribution of the gene sets -> little
	% Likely for some gene sets of other function to be associated 
	% Worthwile to repeat Corsortium 2015a with LDSC
	% Problem with partitioning
	% Can easily be driven by small number of SNPs with large effect
	% Further developments might be required
	
	% Challenges
	% Rare variants -> Rare
	% Need large sample size to obtain sufficient power
	% Annotation of nonexonic regions
	% Variation in non-complex region (alignment problem)
	...
	With the rapid advancement of technologies, it is now possible to perform association studies on genome-wide scale using \gls{GWAS}.
	After years of failure in research of \glng{scz} genetics, the \Glng{scz} Working group of \gls{pgc} has finally identified 108 genetic loci associated with \glng{scz} \citep{Ripke2014}.
	...
	Using the latest \gls{GWAS} summary statistics released by the \gls{pgc}, we estimated that \glng{scz} has a \gls{SNP}-heritability of 0.185 (SD=0.00450), which is similar to the estimate of 0.198 (SD=0.0057) by \gls{ldsc}.
	...
	On the other hand, we also performed an RNA sequencing on the \gls{polyic} \gls{mia} mouse model to investigate if differential gene expression induced by \gls{mia} and genetic variations observed in \glng{scz} were acting on the same functional pathways / gene sets in the development of \glng{scz}.
