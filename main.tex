...
\setlength{\parskip}{0.75em}
	...
	To understand whether if \glng{scz} was more affected by variance in genetic factors or environmental factors, we will need to estimate the its heritability.
	...
	50 matched control were also recruited to the studi.
	...
	Despite the usefulness of adoption studies in separating the effect of shared environment from the genetic factors, collection of adoption data were difficult. 
	On the other hand, any prenatal influence such as alcohol abuse during pregnancy might confound the results.
	Therefore, another useful approach would be the twin studies using the relationship between the \gls{mz} and \gls{dz} twins.
	...
	By combining Falconer's formula and the concept of liability threshold model, \citet{Gottesman01071967} estimated that the heritability of \glng{scz} should be $>60\%$ previously collected twin data.
	These result strongly suggest \glng{scz} as a genetic disorder.
	
	Such finding was further supported by one of the landmark meta-analysis study conducted by \cite{Sullivan2003}.
	Based on data obtained from 12 published \glng{scz} twin studies, the authors found that although there was a non-zero contribution of environmental influence on liability of \glng{scz} ($11\%$,\gls{ci}=$3\%-19\%$), there was a much larger contribution from genetics ($81\%$, \gls{ci}=$73\%-90\%$), suggesting that \glng{scz} was largely mediated by the genetic elements.
	...
	By comparing the observed life time morbid risk and the calculated risk from different models, it was observed that the model with a maximum risk of $\sim\le2$ was more compatible than that with risk $\ge3$\citep{Risch1990}.
	...
	\gls{LD} was of particular importance for it was the non-random correlation of genotypes between 2 genetic locus. 
	\glspl{SNP} in high \gls{LD} were usually observed together in the human genome.
	When a large amount of \glspl{SNP} were in high \gls{LD} together, they form what was known as an \gls{LD} block.
	So if one can select a representative \gls{SNP} within the \gls{LD} block and only perform association testing on the representative \gls{SNP}.
	This was considered as ``tagging'' and this allow researchers to only test a subset of \glspl{SNP} instead of needing to sequence the whole genome.
	
	Such concept form the fundamental basis of \gls{GWAS} which was now extensively used in the genetic research.
	
	\subsection{Genome Wide Association Study}
	% Now go into the GWAS part
	...
	% GxE interaction? Found no nonadditive genetics but didn't test the GxE
