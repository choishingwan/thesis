...
\mainmatter
	...
	A total of 16 samples passed the quality control with age ranged from \gls{gd}8 to 23 years old. 
	...
	As there are insufficient samples for the construction of gene co-expression network for individual developmental stage, we try to construct a co-expression network for genes across all developmental stage. 
	...
	Finally, there were 17,168 genes passing through the quality threshold and were used for the construction of co-expression network. 
	 
	\subsubsection{Network Construction}
	\gls{wgcna} (ver 1.47) were used for the construction of gene co-expression network\citep{Langfelder2008}. 
	The \emph{blockwiseModules} function, using Biweight Midcorrelation for the construction of correlation matrix and a restriction of minimum model size of 30. 
	Soft-power threshold were set to 15 where it is the first threshold value which has $R^2 > 0.8$ (0.817) and the $R^2$ is saturated\citep{Zhang2005}(\cref{fig:softpowerThreshold}).
	...
	Module eigen-genes were calculated as the first \gls{pc} of expressions of the genes within individual modules using the \emph{moduleEigengenes} function from \gls{wgcna}. 
	...
	Finally, correlation between age and module eigen-gene expression were calculated pearson correlation.
	...
	Genes within the modules were provide as the target gene lists and all the genes passed quality controls were used as the background gene list.
	...
	A total of 35 networks were constructed with a mean network size of 421.6. 
	Of the 35 modules, 9 have a p-value $< 0.05$ and only one module, the ``black'' module passed the bonferroni correction threshold (p-value $<0.00143$) (\cref{tab:moduleSig}).
	\begin{table}[htbp]
		\centering
		...
		\label{tab:moduleSig}%
	\end{table}%	
	It is important to understand the gene expression pattern of this significant module with respect to developmental age. 
	As a result of that, we use all available hippocampus samples from BrainSpan (n=32) and plot the mean log$_2$ \gls{rpkm} value against the developmental age (\cref{fig:blackMod}).
	A line of best fit was calculated using the \emph{stat\_smooth} with the loess function from R package \emph{ggplot2}(version 1.0.1). 
	It was observed that there is a clear trend of gene expression with respect to developemental age with the gene expression increase from early gestation and peak at around late adolescence ($\approx$18 years).
	Interestingly, this peak in expression seems to concur with the common age of onset of schizophrenia.%Need citation here. 
	Although this network were built on using expression data of the normal samples, it might provide insight into the development of the disorder.
	
	\subsubsection{Functional Annotation}
	Upon performing the \gls{GO} enrichment analysis, it was found that the significant module was enriched for genes involve in ``central nervous system development'' (p-value=$5.94\times 10^{-5}$),  ``carboxylic acid metabolism'' (p-value=$4.92\times 10^{-6}$), ``positive regulation of production of molecular mediator of immune response'' (p-value=$6.21\times 10^{-3}$), ``response to biotic stimulus'' (p-value=$6.14\times 10^{-3}$) and ``cytolysis'' (p-value=$8.72\times 10^{-3s}$).
	The enrichment of ``central nervous system development'' supports the importance of this module's role in the development of hippocampus. 
	Similarly, one of the \gls{GO} term represented by ``carboxylic acid metabolism'' is the ``glutamate metabolic process''(p-value=$3.54\times 10^{-3}$).
	Consider the importance of glutamate in mediating brain development\citep{Lujan2005}, the enrichment in ``glutamate metabolic process'' further supports module's developmental role in hippocampus. 
	...
	\begin{figure}
		...
		\centering
		\scalebox{.8}{\includegraphics{figure/Network_Expression.png}}
		\label{fig:blackMod}
	\end{figure}
