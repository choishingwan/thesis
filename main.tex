...
\mainmatter
	...
	For each developmental stage, we select the sample with a dissection score $\ge3$ and an \gls{rin} $\ge8$. 
	...
	Soft-power threshold were set to 13 where it is the first threshold value which has $R^2 \approx 0.8$ (0.795) and the $R^2$ is saturated\citep{Zhang2005}(\cref{fig:softpowerThreshold}).
	
	\begin{figure}
		\caption[Soft-power threshold selection]{Soft-power threshold selection. A soft-power of 13 were selected as it is the first threshold value having $R^2\approx0.8$ (0.795) and where the $R^2$ is saturated.}
		\centering
		\scalebox{.8}{\includegraphics{figure/SoftpowerThreshold.png}}
		\label{fig:softpowerThreshold}
	\end{figure}
	\subsection{Result}
	\subsubsection{Sample Quality Controls and Normalization of data}
	\section{Discussion}
