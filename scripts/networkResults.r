...
gg=ggplot(plotData, aes(x=Age, y=expression))+geom_point()+labs(x = "Age", y = "Mean log2 PRKM")+theme_classic(base_size = 12)+ theme(axis.text.x = element_text(angle = 50, hjust = 1))+ ggtitle("Mean Expression of Genes in \nthe Co-expression network")+ theme(plot.title = element_text(lineheight=.8, face="bold",size=24),axis.text=element_text(size=16),axis.title=element_text(size=16,face="bold"))+ stat_smooth(method = "loess", formula = y ~ x, size = 0.5,colour="black", alpha = 0.2)+scale_x_discrete(labels = plotData$age, breaks=plotData$Age)
print(gg)
dev.off()
}
