...
Over 90$\%$ of reads were uniquely mapped to the \organism{Mus musculus} reference genome (mm10, Ensembl GRCm38.74) and the ERCC spike-in control reference using STAR\cite{Dobin2013}. 
...
Many of our differentially expressed genes (DEGs) were within the list of schizophrenia-related candidate genes identified by the Psychiatric Genomics Consortium (PGC)\cite{Greenwood2011} previously (hyper geometric p-value $= 1.12\times 10^{-11}$) (Table \ref{tab:candidateGeneTable}), including \gene{Dlg4}\cite{Balan2013} and \gene{Disc1}\cite{Morris2003}.
Some DEGs were also known to be associated with autism (like \gene{Gabrb3}\cite{Fatemi2009}) or other brain diseases (like \gene{Chrnb2} in epilepsy\cite{Conti2015}).
\begin{figure}[!h]
	\centering     
...
Seven of the candidate schizophrenia genes previously identified by the PGC remained in our DEG list, including the GABA enzyme, \gene{Gad1} and \gene{Gad2}; \gene{Gabra3}, \gene{Ctnna2} and the dopamine-related gene, \gene{Th} (Table \ref{tab:candidateGeneTable}). 
...
\input{environmental_risk/er_candidateGeneTable}
\input{environmental_risk/er_geneOntologyTable}
\input{environmental_risk/er_targetGeneSetEnrichment}
