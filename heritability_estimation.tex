...
% Need to stress that we are only calculating the narrow sense heritability
	...
	\section{Result}
		...
		Although the number of causal \glspl{SNP} did not have a significant impact to the performance of \gls{shrek}, a general up-ward bias was observed. 
		As the simulated heritability increases, the bias of the estimate systematically increases.
		One possible reason for the general up-ward bias was that we under-estimate the $R^2$ during the analysis. 
		When considering \cref{eq:rho2Correct}, it was observed that the $R^2$ were downwardly corrected. 
		Therefore we hypothesize that by not performing the bias correcting in the \gls{LD}, we might get more accurate estimation of the heritability.
		
		We re-run the simulation on \cref{sec:shrekSim}, this time comparing the performance of \gls{shrek} with and without the \gls{LD} correction.
		It was observed that when \gls{shrek} was performed without \gls{LD} correction, there was a downward bias. 
		The magnitude of bias was much smaller when compared to the estimation with \gls{LD} correction. 
		On top of that, the variance of estimation was also smaller when \gls{LD} correction was not performed.
