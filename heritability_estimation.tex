...
% Need to stress that we are only calculating the narrow sense heritability
	...
	\gls{LD} score was computed based on the 446 samples using a 1\gls{mb} window size.
	...
	\section{Result}
		...
		Yet from the simulation results, it suggests that the intercept estimation might have corrected some bias, leading to a better performance when intercept estimation was performed. 
		
		Another interesting observation was that although the \gls{mse} tends to decrease when the population prevalence increases for most algorithms, an inversed relationship was observed for \gls{ldsc} when intercept estimation was performed. 
		As the population prevalence increases, the \gls{mse} also increases for \gls{ldsc} with intercept estimations and further investigation might be required to understand the specific reason of this behavior.
		...
		\begin{table}
			...
			\caption[MSE of Case Control Simulation]{
				...
				Further investigation might be required to understand the reversed behavior of \gls{ldsc} with intercept estimation when compared to other algorithms, especially when the same pattern was not observed for \gls{ldsc} with fixed intercept estimation. 
				On the other hand, one can observed that the \gls{mse} tends to decrease as the number of causal \glspl{SNP} increases, as observed in the quantitative simulation. 
			}
			\label{tab:mseCC}
		\end{table}
		...
		%TODO at the end, it is important to state the estimation problem
		\subsection{Real Data Simulation}
		
		\subsection{Application to Real Data}
		We applied our methods to the \gls{pgc} \gls{scz}, major depression disorder, autism and bipolar data sets.
		
	\section{Discussion}
	In this chapter, we introduced an alternative method to \gls{ldsc} for the estimation of heritability using only the test statistics and have made available for download a standalone C++ programme \gls{shrek}.
	
	\subsection{LD Correction}
	As \gls{shrek} relies on the \gls{LD} information to estimate the heritability, it is important for us to correct for possible sampling bias form the \gls{LD} matrix. 
	When testing for the bias correction algorithms, it was observed that majority of the algorithms, except that of \cref{eq:weir}, inflates the heritability estimated. 
	Therefore, our initial decision was to use \cref{eq:weir} as our \gls{LD} sampling bias correction algorithm.
	However, in subsequent simulation of the quantitative traits, a general overestimation of the heritability was again observed.
	Because the pattern of overestimation is close to the one observed in the \gls{LD} correction simulation, we hypothesize \cref{eq:weir} might also have introduced bias into the heritability estimation but the bias is only noticeable when the number of \glspl{SNP} increases.
	When compared the \gls{shrek}, \gls{ldsc} only overestimates the heritability when the number of causal \glspl{SNP} is small.
	When inspecting their algorithm, it was observed that \gls{ldsc} also corrected for the sampling bias on the $R^2$, but with yet another equation:
	\begin{equation}
	\text{LDSC}: \tilde{R^2}= \hat{R^2}-\frac{1-\hat{R^2}}{n-2}\label{eq:ldscR2} \\
	\end{equation}
	It is therefore important to test whether if the application of \cref{eq:ldscR2} might help to improve the performance of \gls{shrek} and whether if the performance of \cref{eq:weir} does decrease when the number of \glspl{SNP} increases.
	We therefore repeated the simulation on \gls{LD} correction but instead of simulating 5,000 \glspl{SNP} on chromosome 22, we simulated 50,000 \glspl{SNP} on chromosome 1.
	...
	Surprisingly, when large amount of \glspl{SNP} were estimated, the bias of heritability estimation was the smallest when no \gls{LD} correction was performed (\cref{fig:ldCorBigCom}) where only a small downward bias was observed. 
	The performance of \cref{eq:ldscR2} was comparable to \cref{eq:pratt} and \cref{eq:weir} introduce least bias to the estimation.
	It was clear that the genomic data structure (e.g. 0,1,2) violates some of the underlying assumption of the \gls{LD} correction algorithms (e.g. normality) thus general \gls{LD} correction algorithms might not be applicable to our situation.
	Further research is therefore required to find an optimal solution to correct for the \gls{LD} sampling bias. 
