...
% Need to stress that we are only calculating the narrow sense heritability
	...
	\section{Comparing Different LD correction Algorithms}
		...
		In order to assess the performance of each individual correction methods, we perform a simple quantitative polygenic trait simulation to compare the performance of our algorithm using different \gls{LD} bias correction algorithms.
		...
		\subsection{Number of SNPs in Simulation}
			...
			%TODO the snp density of our simulation
	...
	\section{Result}
		...
		\begin{figure}
			\centering
			\subfloat[Mean of Estimation without LD Correction]{
				\scalebox{.4}{\includegraphics{figure/quantitative/ld_correct/shrek_Qt_Rand_mean.png}}
				\label{fig:shrekNCMean}
			}
			\subfloat[Mean of Estimation with LD Correction]{
				\scalebox{.4}{\includegraphics{figure/quantitative/ld_correct/shrekC_Qt_Rand_mean.png}}
				\label{fig:shrekCMean}
			}\\
			\subfloat[Variance of Estimation without LD Correction]{
				\scalebox{.4}{\includegraphics{figure/quantitative/ld_correct/shrek_Qt_Rand_sd.png}}
				\label{fig:shrekNCVar}
			}
			\subfloat[Variance of Estimation with LD Correction]{
				
				\scalebox{.4}{\includegraphics{figure/quantitative/ld_correct/shrekC_Qt_Rand_sd.png}}
				\label{fig:shrekCVar}
			}
		...
		The result does suggest that there might be some problem with the \gls{LD}, therefore lead to bias in estimation when the \gls{LD} correction was used. 
		...
		In conclusion, \gls{shrek} without \gls{LD} correction seems to be superior in performance when compared to \gls{shrek} with \gls{LD} correction. %TODO The mse table for SHREK 
		...
		%TODO might need to explain whey LD correction is bad here later after we have figure it out
		...
		First, we would like to examine the mean heritability estimation when compared to the simulated heritability.
