...
% Need to stress that we are only calculating the narrow sense heritability
	...
	\section{Simulation}
		...
		The time required for simulation increase as number of \glspl{SNP} increases. 
		...
		\begin{enumerate}
			...
			\item Repeat step 1-6 10 times
		\end{enumerate}
		\subsubsection{Effect of Minor Allele Frequency}
		...
		Given limited computer resources, we only simulate 1,000 cases, with an observed prevalence of 0.5 and population prevalence $p\in\{0.5,0.001\}$.
		...
		The simulation of extreme phenotype selection follows that of the quantitative trait simulation. 
		Due to their similarity, we only simulate condition where there were 100 causal \glspl{SNP}. 
		We will then simulate 5,000 samples and select 500 samples from the extreme end of the sample distribution.
