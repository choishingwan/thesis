...
% Need to stress that we are only calculating the narrow sense heritability
	...
	To demonstrate that \gls{shrek} also works outside of simulated data, we also estimated the \gls{SNP} heritability of \glng{scz} and other psychiatric disorders using \gls{shrek}.
	...
	\section{Methodology}			
		\subsection{Heritability Estimation}
			...
			A challenge in calculating the heritability from \gls{GWAS} data is that usually only the summary statistic or p-value are provided.
		...
		%TODO have not figure out the logic behind those equation 
		%Discuss on the liability threshold model. Then the apply orange paper. Then explain how to get the results. 
			...
			Based on the derivation of \citet{Yang2010}, the approximate ratio between the summary statistic obtained from case control studies ($\chi^2_{CC}$) and quantitative trait studies($\chi^2_{QT}$) is
...
%			
	\subsection{Comparing Different LD correction Algorithms}
		...
		In order to assess the performance of each individual correction methods, simulations were performed.
		...
		The whole process were repeated 50 times such that a distribution of the estimates can be obtained. 
...
%			However, to simplify our simulation, all ``causal'' variants were included in our simulation (e.g. perfectly ``tagged'').
			...
			For each population, the whole process were repeated 50 times such that a distribution of the estimate can be obtained. 
		...
		Phenotype of the individuals were then simulated using \cref{eq:simulationOfPhenotype} and were standardized. %TODO unresolved, K need better explaination
...
%	\end{enumerate}
	...
	Therefore estimates from our algorithm can only serve as an upper bound of the true \gls{SNP} heritability of \glng{scz}.
